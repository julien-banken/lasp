...
init([]) ->
    logger:log(notice,"Divergence server initialized."),
    ...
    case lasp_workflow:is_task_completed(events) of
        true ->
            case lasp_workflow:is_task_completed(anti_entropy, 1) of
                false ->
                    logger:log(notice,"Performing anti-entropy pass with all clients."),
...
log_divergence() ->
    ...
    logger:log(notice,"**** Divergence information."),
    logger:log(notice,"**** Value: ~p", [Value]),
    logger:log(notice,"**** MaxEvents: ~p", [MaxEvents]),
    logger:log(notice,"**** Overcounting: ~p", [Overcounting]),
    logger:log(notice,"**** OvercountingPercentage: ~p", [OvercountingPercentage]),
